...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT a.act_id,a.act_name,a.act_count,g.goods_id,g.goods_thumb2 '.
            'FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' AS a, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            "WHERE a.goods_id=g.goods_id and g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.brand_id = '$brand_id'".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['url']           = build_uri('auction', array('auid' => $row['act_id']));
        $sql="select click_count from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$row['goods_id'];
        $arr[$row['goods_id']]['count']=$GLOBALS['db']->getOne($sql);
    }
    return $arr;
}
