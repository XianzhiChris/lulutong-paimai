...
function index_get_auction()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr['chujia_count']=$GLOBALS['db']->getOne("select count(*) from ". $GLOBALS['ecs']->table('auction_log') ." where act_id='".$row['act_id']."'");
        $arr['chujia']=$chujia?price_format($chujia):price_format($ext_info['start_price']);
        $list[] = $arr;
    }
