...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    //$smarty->assign('best_goods',      get_recommend_goods('best'));    // 精品展示  放入recommend_best文件中
    ...
    /*拍卖 李云鹏20160426*/
    $smarty->assign('today_auction',     index_get_auction());       // 今日拍品
    $smarty->assign('banner',     index_get_auction());       // 全屏广告
    ...
    /* links */
    $links1 = index_get_links(1);  //首页指导机构  李云鹏 20160426
    $links2 = index_get_links(2);  //首页合作机构  李云鹏 20160426
    $links3 = index_get_links(3);  //首页入驻机构  李云鹏 20160426

    $smarty->assign('links1',       $links1);
    $smarty->assign('links2',       $links2);
    $smarty->assign('links3',       $links3);
...
function index_get_auction()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr['thumb'] = get_image_path($row['goods_id'], $row['goods_img2'], true);
 ...
 * @return  array
 */
function index_get_links($link_cat)
{
    $sql = 'SELECT link_logo, link_name, link_url FROM ' . $GLOBALS['ecs']->table('friend_link') . ' where link_cat='.$link_cat.' ORDER BY show_order';
    $res = $GLOBALS['db']->query($sql);

    while ($row = $GLOBALS['db']->fetchRow($res))
    {

            $links[] = array('name' => $row['link_name'],
                                    'url'  => $row['link_url'],
                                    'logo' => $row['link_logo']);
