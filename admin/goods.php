...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        /* 商品长方形图片路径 李云鹏20160503*/
        if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 10) && !empty($goods['original_img2']))
        {
            $goods['goods_img2'] = get_image_path($_REQUEST['goods_id'], $goods['goods_img2']);
            $goods['goods_thumb2'] = get_image_path($_REQUEST['goods_id'], $goods['goods_thumb2'], true);
        }
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /* 检查图片：如果有错误，检查尺寸是否超过最大值；否则，检查文件类型 长方形图片 李云鹏20160503*/
    if (isset($_FILES['goods_img2']['error'])) // php 4.2 版本才支持 error
    {
        // 最大上传文件大小
        $php_maxsize = ini_get('upload_max_filesize');
        $htm_maxsize = '2M';
        ...
        // 商品图片
        if ($_FILES['goods_img2']['error'] == 0)
        {
            if (!$image->check_img_type($_FILES['goods_img2']['type']))
            {
                sys_msg($_LANG['invalid_goods_img'], 1, array(), false);
            }
        }
        elseif ($_FILES['goods_img2']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['goods_img_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['goods_img2']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['goods_img_too_big'], $htm_maxsize), 1, array(), false);
        }
    ...
    {
        // 商品图片
        if ($_FILES['goods_img2']['tmp_name'] != 'none')
        {
            if (!$image->check_img_type($_FILES['goods_img2']['type']))
            {
    ...
    $goods_img2        = '';  // 初始化长方形图片   李云鹏20160426
    $goods_thumb2      = '';  // 初始化长方形图片缩略图
    $original_img2     = '';  // 初始化长方形原始图片
    ...
    // 如果上传了商品长方形图片，相应处理  李云鹏20160503
    if ($_FILES['goods_img2']['tmp_name'] != '' && $_FILES['goods_img2']['tmp_name'] != 'none')
    {
        $original_img2   = $image->upload_image($_FILES['goods_img2']); // 长方形原始图片 李云鹏20160426
        if ($original_img2 === false)
        {
            sys_msg($image->error_msg(), 1, array(), false);
        }
        $goods_img2      = $original_img2;   // 长方形商品图片  李云鹏20160426

        // 如果系统支持GD，缩放商品图片，且给商品图片和相册图片加水印
        if ($proc_thumb && $image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img2']['type']))
        {
            $goods_img2 = $image->make_thumb('../'. $goods_img2 , 710, 240);  //长方形缩略图  李云鹏20160426
            if ($goods_img2 === false)
            {
                sys_msg($image->error_msg(), 1, array(), false);
            }
            //拍卖-长方形缩略图--李云鹏20160426
            $goods_thumb2 = $image->make_thumb('../' . $original_img2, 592,  200);
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, is_buy,buymax,buymax_start_date,buymax_end_date, goods_img,goods_img2, goods_thumb,goods_thumb2, original_img,original_img2, keywords, goods_brief, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date','$goods_img','$goods_img2', '$goods_thumb','$goods_thumb2', '$original_img','$original_img2', ".
    ...
    else
    {
        ...
        if ($goods_img2)
        {
            $sql .= "goods_img2 = '$goods_img2', original_img2 = '$original_img2',goods_thumb2 = '$goods_thumb2',  ";
        }
        if ($goods_thumb)
        {
    ...
    $original_img2 = reformat_image_name('goods2', $goods_id, $original_img2, 'source'); //长方形缩略图--李云鹏20160426
    $goods_img2 = reformat_image_name('goods2', $goods_id, $goods_img2, 'goods');
    $goods_thumb2 = reformat_image_name('goods_thumb2', $goods_id, $goods_thumb2, 'thumb');
...
//长方形缩略图--李云鹏20160426
    if ($goods_img2 !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET goods_img2 = '$goods_img2' WHERE goods_id='$goods_id'");
    }

    if ($original_img2 !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET original_img2 = '$original_img2' WHERE goods_id='$goods_id'");
    }
    if ($goods_thumb2 !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET goods_thumb2 = '$goods_thumb2' WHERE goods_id='$goods_id'");
    }
