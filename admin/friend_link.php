...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($exc->num("link_name", $link_name) == 0)
    {
        ...
        /* 插入数据 */
        $sql    = "INSERT INTO ".$ecs->table('friend_link')." (link_name, link_url, link_logo, show_order,link_cat) ".
                  "VALUES ('$link_name', '$link_url', '$link_logo', '$show_order',$link_cat)";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取得友情链接数据 */
    $sql = "SELECT link_id, link_name, link_url, link_logo, show_order,link_cat ".
...
function get_links_list()
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        ...
        switch ($rows['link_cat']){
            case 1:
                $rows['link_cat_name']="指导机构";
                break;
            case 2:
                $rows['link_cat_name']="合作机构";
                break;
            case 3:
                $rows['link_cat_name']="入驻机构";
                break;
        }
