...
//     $datainfo['goods']     = $db->GetOne('SELECT COUNT(goods_id) FROM ' . $ecs->table('goods'));
    ...
    //拍卖商品总数  李云鹏20160510
    $datainfo['auction']     = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('goods_activity'));
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    $today = array();
    //获取短信接口  李云鹏20160430
    $sms_v=$_CFG['sms_shop_mobile'];
    $today['sms_v']=$sms_v;
    //获取短信余额  只有云片支持 李云鹏20160430
    require_once (ROOT_PATH . 'sms/sms.php');
    if($sms_v==1){
        $today['sms_yue']=syYunSMS();
    }
    //今日销售总额
...
//今日注册会员
    ...
    //待处理总数
    //$task['total'] = $task['commission']+$task['goods']+$task['deposit']+$task['withdraw']+    //去掉商品数  李云鹏20160520
        //$task['message']+$task['comment']+$task['shared']+$task['tag'];
    $task['total'] = $task['commission']+$task['deposit']+$task['withdraw']+
        $task['message']+$task['comment']+$task['shared']+$task['tag'];
    //var_dump($task['commission']."|".$task['goods']."|".$task['deposit']."|".$task['withdraw']."|".
        //$task['message']."|".$task['comment']."|".$task['shared']."|".$task['tag']);
    ...
    //拍卖商品总数  李云鹏20160510
    $goods['auction'] = $db->getOne(' SELECT COUNT(*) FROM '.$ecs->table('goods_activity'));
    //库存警告商品总数
    if ($_CFG['use_storage'])
    {
