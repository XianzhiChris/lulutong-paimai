...
/**
 * 取得未开始拍卖活动  李云鹏 20160601
 * @return  array
 */
function get_yupai_auction()
{
    $now = gmtime();
    $limit = get_library_number('auction', 'index');
    $sql = "SELECT a.act_id, a.goods_id, a.act_name, a.ext_info,a.end_time,a.start_time, g.goods_thumb, g.goods_img2,g.supplier_id,g.brand_id ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
            sub_str($arr['act_name'], $GLOBALS['_CFG']['goods_name_length']) : $arr['act_name'];
        $arr['short_style_name']   = add_style($arr['short_name'],'');
        //出价次数
        $arr['chujia_count']=$GLOBALS['db']->getOne("select count(*) from ". $GLOBALS['ecs']->table('auction_log') ." where act_id='".$row['act_id']."'");//最后出价价格
        ...
        //送拍人：商家
        //$arr['songpairen']=$GLOBALS['db']->getOne("select supplier_name from ". $GLOBALS['ecs']->table('supplier') ." where supplier_id='".$row['supplier_id']."'");
        //送拍机构:品牌  用品牌代替送拍机构  李云鹏20160509
        $arr['songpairen']=$GLOBALS['db']->getOne("select brand_name from ". $GLOBALS['ecs']->table('brand') ." where brand_id='".$row['brand_id']."'");
        //结束时间：倒计时
        $arr['end_time']=$row['end_time'];
        //开始时间：
        $arr['start_time']=$row['start_time'];
        $arr['format_start_time']=local_date("m-d H:i",$row['start_time']);
...
/**
 * 获得指定分类下的商品   改为拍卖分类商品  李云鹏20160426
 *
 * @access  public
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = "SELECT a.act_id, a.goods_id, a.act_name, a.ext_info,a.end_time,a.start_time,a.act_count,a.is_finished, g.goods_thumb2, g.goods_img2,g.supplier_id,g.brand_id ".
        "FROM " . $GLOBALS['ecs']->table('goods_activity') . " AS a," .
        $GLOBALS['ecs']->table('goods') . " AS g" .
        " WHERE a.goods_id = g.goods_id" .
        " AND a.act_type = '" . GAT_AUCTION . "'" .
        ...
        " ORDER BY a.end_time desc,a.act_id desc";
    if ($num > 0)
    {
        $sql .= ' LIMIT ' . $num;
    }
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['id']           = $row['act_id'];
        $goods[$idx]['name']         = $row['act_name'];
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
            sub_str($row['act_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['act_name'];
        $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb2'], true);
        $goods[$idx]['url']          = build_uri('auction', array('auid' => $row['act_id']));
        //点击次数
        //$goods[$idx]['act_count']=$row['act_count'];
        $sql="select click_count from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$row['goods_id'];
        $goods[$idx]['act_count']=$GLOBALS['db']->getOne($sql);
        //出价次数
        $goods[$idx]['chujia_count']=$GLOBALS['db']->getOne("select count(*) from ". $GLOBALS['ecs']->table('auction_log') ." where act_id='".$row['act_id']."'");
        //送拍人：商家
        //$goods[$idx]['songpairen']=$GLOBALS['db']->getOne("select supplier_name from ". $GLOBALS['ecs']->table('supplier') ." where supplier_id='".$row['supplier_id']."'");
        //送拍人:品牌  用品牌替代送拍机构  李云鹏20160509
        $goods[$idx]['songpairen']=$GLOBALS['db']->getOne("select brand_name from ". $GLOBALS['ecs']->table('brand') ." where brand_id='".$row['brand_id']."'");
        //结束时间：倒计时
        $goods[$idx]['end_time']=$row['end_time'];
        //开始时间：剩余倒计时
        $goods[$idx]['start_time']=$row['start_time'];
    }
...
function auction_info($act_id, $config = false)
{
    ...
    $auction['mk_start_time'] = $auction['start_time'];  //加入时间戳格式  李云鹏20160510
    $auction['mk_end_time'] = $auction['end_time'];
    if ($config == true)
    {
...
function auction_log($act_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $row['formated_bid_time'] = local_date("H:i", $row['bid_time']);
        $row['bid_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['bid_time']);
        $row['formated_bid_price'] = price_format($row['bid_price'], false);
        $row['user_name']="****".substr($row['user_name'],-4);  //截取用户名后四位 李云鹏20160428
        $log[] = $row;
    }
...
function auction_status($auction)
{
    ...
    if ($auction['is_finished'] == 0)
    {
        ...
        {
            return PRE_START; // 0;未开始
        }
        elseif ($now > $auction['end_time'])
        {
            return FINISHED; // 2;已结束，未处理
        }
        else
        {
            return UNDER_WAY; // 1;进行中
        }
    }
