...
class cls_template
{
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            /* 在头部加入版本信息 */
            $source = preg_replace('/<head>/i', "<head>\r\n<meta name=\"author\" content=\"先知教育-IT培训 QQ:276906866\" />",  $source);
			/* By www.68ecshop.com 代码增加_start */
			$source = preg_replace('/<head>/i', "<head>\r\n<base href=\"". $GLOBALS['ecs']->url() ."\" />",  $source);
			/* By www.68ecshop.com 代码增加_end */
            //底部加入先知教育友情链接  李云鹏 20160426
            $source = preg_replace('/<\/body>/i', "<a href=\"http://www.itmba.cc\" style=\"display:none\">先知教育 淄博PHP培训</a>\r\n</body>",  $source);
