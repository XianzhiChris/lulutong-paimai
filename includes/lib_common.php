 ...
 */
function supplier_info($suppid){
    $sql = "select * from ". $GLOBALS['ecs']->table('supplier'). " where supplier_id=".$suppid;
    $result = $GLOBALS['db']->getRow($sql);

    $sql="select count(*) from ". $GLOBALS['ecs']->table('goods_activity'). " where supplier_id=".$suppid;
    $num=$GLOBALS['db']->getOne($sql);
    $result['total']=$num;
    return $result;
}
/**
 * 查询商品品牌信息  李云鹏20160509
...
function brand_info($brand_id){
    if($brand_id){
        ...
        $sql="select count(*) from ". $GLOBALS['ecs']->table('goods'). " where brand_id = '$brand_id' AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0";
        $num=$GLOBALS['db']->getOne($sql);
        $result['total']=$num;
        return $result;
    }else{
