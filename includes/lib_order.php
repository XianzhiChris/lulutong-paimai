...
function order_info($order_id, $order_sn = '')
{
    /* 计算订单各种费用之和的语句 */
    //加入拍卖佣金 李云鹏20160522
    $total_fee = " (goods_amount - discount + tax + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + yongjin_fee) AS total_fee ";
    ...
    if ($order)
    {
        ...
        $order['formated_yongjin_fee']       = price_format($order['yongjin_fee'], false);   //加入拍卖佣金 李云鹏20160522
...
function order_fee($order, $goods, $consignee)
{
    ...
    $total  = array('real_goods_count' => 0,
                    ...
                    'pack_fee'         => 0,
                    'card_fee'         => 0,
                    'yongjin_fee'         => 0,  //增加拍卖佣金 10% 李云鹏20160521
                    'shipping_fee'     => 0,
                    'shipping_insure'  => 0,
                    'integral_money'   => 0,
                    'bonus'            => 0,
    ...
    /* 拍卖佣金费用 李云鹏20160521 佣金计算方式为商品价格-优惠券=实际支付价格x10% 20160522*/
    if (!empty($GLOBALS['_CFG']['yongjin_user']))
    {
        $total['yongjin_fee']      = ($total['goods_price']-$total['bonus'])*($GLOBALS['_CFG']['yongjin_user']/100);
    }
    $total['yongjin_fee_formated'] = price_format($total['yongjin_fee'], false);
    ...
    {
            //增加拍卖佣金 李云鹏20160521 yongjin_fee
        $total['amount'] = $total['goods_price'] - $total['discount'] + $total['tax'] + $total['pack_fee'] + $total['card_fee'] +
            $total['yongjin_fee'] + $total['shipping_fee'] + $total['shipping_insure'] + $total['cod_fee'];
...
//				" AND supplier_id = ".$key.
//				" AND min_amount > 0 ";
            //更改优惠券为购物金额的30%  李云鹏20160521  优惠券金额要满足10,并且要取整,比如18,取整为10
            $sql = "SELECT  floor(FLOOR('$val' / 10) * 0.3)*10 " .
                "FROM " . $GLOBALS['ecs']->table('bonus_type') .
                " WHERE send_type = '" . SEND_BY_ORDER . "' " .
                " AND send_start_date <= '$today' " .
                "AND send_end_date >= '$today' " .
...
function order_amount_field($alias = '')
{
    ...
    return "   {$alias}goods_amount + {$alias}tax + {$alias}shipping_fee" .
           ...
           " + {$alias}card_fee + {$alias}yongjin_fee ";
}
...
//                    "VALUES('$bonus[type_id]', '$user[user_id]', '$supplier_id')";
            //插入当前时间戳作为 李云鹏20160521

            for ($i = 0; $i < $bonus['number']; $i++)
            {
                $bonus_sn=time().rand(100,999);
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('user_bonus') . " (bonus_type_id, user_id, supplier_id,bonus_sn) " .
                    "VALUES('$bonus[type_id]', '$user[user_id]', '$supplier_id',$bonus_sn)";
...
//            "AND send_end_date >= '$order_time' ";
    //更改发放优惠券为购物金额的30%  李云鹏20160521
    $sql = "SELECT type_id, type_money, IFNULL(FLOOR(FLOOR('$amount' / 10)*0.3)*10/type_money, 1) AS number " .
        "FROM " . $GLOBALS['ecs']->table('bonus_type') .
        "WHERE send_type = '" . SEND_BY_ORDER . "' " .
        "AND send_start_date <= '$order_time' " .
        "AND send_end_date >= '$order_time' ";
