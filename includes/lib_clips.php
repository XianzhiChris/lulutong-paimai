 ...
 * @return  array
 */
function get_user_auction ($user_id)
{
    $prompt = array();
    $idx = 0;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $act_id = $row['act_id'];
        $auction = auction_info($act_id);  //竞拍商品信息

        $prompt[$idx]['act_name'] = $auction['act_name'];  //商品名称
        $prompt[$idx]['start_price']= $auction['formated_start_price'];  //商品起拍价
        $prompt[$idx]['end_time']= local_date("m-d H:i", $auction['mk_end_time']);;  //竞拍结束时间
        $prompt[$idx]['status_no']= $auction['status_no'];  //竞拍时间状态  0;未开始;1;进行中;2;已结束
        $prompt[$idx]['url'] = build_uri('auction', array('auid' => $act_id));
        //获取当前用户最后一次出价记录
        $sql = "SELECT * " .
            "FROM " . $GLOBALS['ecs']->table('auction_log').
        ...
        $prompt[$idx]['my_bid_time']= local_date("m-d H:i", $log['bid_time']);   //我的最后出价时间
        $prompt[$idx]['my_bid_price']= $log['bid_price'];  //我的最后出价价格

        $prompt[$idx]['bid_time']= local_date("m-d H:i", $auction['last_bid']['bid_time']);   //最后出价时间
        $prompt[$idx]['bid_price']= $auction['last_bid']['bid_price'];  //最后出价价格
        if($auction['status_no']==1){
            $prompt[$idx]['text'] = "<i>进行中</i>";
        }elseif (isset($auction['last_bid']) && $auction['last_bid']['bid_user'] == $user_id && $auction['order_count'] == 0 && $auction['status_no']==2)
        {
            $prompt[$idx]['text'] = "<em>竞拍成功</em>";  //如果竞拍成功
        }else{
            $prompt[$idx]['text'] = "已结束";
        }
        $idx++;
