...
else
{
   ...
   /* fulltext_search_add_END_www.68ecshop.com */
    /* 获得符合条件的商品总数 */
        ...
        $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods_activity'). " AS a," .$ecs->table('goods'). " AS g ".
            "WHERE act_type = '" . GAT_AUCTION . "' and a.goods_id = g.goods_id and g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_virtual=0 ".
            "AND (( 1 "  . $keywords2 .") ".$tag_where." )";
        $count = $db->getOne($sql);
    ...
    /* 查询商品 */
        $sql = "SELECT a.act_id, a.act_name, a.ext_info,a.end_time,a.start_time,g.goods_id, g.goods_name, g.market_price, g.click_count, g.goods_number, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".

                "g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_thumb2, g.goods_img, g.goods_brief, g.goods_type ".
            "FROM " . $ecs->table('goods_activity') . " AS a," .$ecs->table('goods'). " AS g ".

            "WHERE act_type = '" . GAT_AUCTION . "' and a.goods_id = g.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_virtual=0 $attr_in $categories ".
                "AND (( 1 " . $keywords2 . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .
    ...
    while ($row = $db->FetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['act_name'] = $row['act_name'];
        $arr[$row['goods_id']]['short_act_name']       = $GLOBALS['_CFG']['goods_name_length'] > 0 ? sub_str($row['act_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['act_name'];

        foreach($replacef_www_68ecshop_com as $key_www_68ecshop_com =>$temp_qq)
        {
            //$replacet[$key_www_68ecshop_com]=  '<strong style="color:#cc0000;">'. $temp_qq .'</strong>';;
            $arr[$row['goods_id']]['short_act_name'] = preg_replace('/(?!<[^>]*)'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['short_act_name']);
        }
        ...
        $arr[$row['goods_id']]['goods_thumb2']   = get_image_path($row['goods_id'], $row['goods_thumb2'], true);
        $arr[$row['goods_id']]['goods_img']     = get_image_path($row['goods_id'], $row['goods_img']);
        $arr[$row['goods_id']]['url']           = build_uri('auction', array('auid' => $row['act_id']), $row['act_name']);
        ...
        //出价次数
        $arr[$row['goods_id']]['chujia_count']=$GLOBALS['db']->getOne("select count(*) from ". $GLOBALS['ecs']->table('auction_log') ." where act_id='".$row['act_id']."'");
        //最后出价价格
        ...
        //开始时间：
        $arr[$row['goods_id']]['start_time']=$row['start_time'];
