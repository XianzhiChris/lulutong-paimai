...
function action_send_mobile_code ()
{
	...
	$result = $GLOBALS['_CFG']['sms_shop_mobile']?sendYunSMS($mobile_phone, $mobile_code):sendJuheSMS($mobile_phone, $mobile_code);  //根据后台设置短信接口发送短信 李云鹏20160429
	if($result)
	{
...
function action_default ()
{
	...
	//获取竞买须知内容 李云鹏20160601
	$jmxz=$db->getOne("select content from ".$ecs->table("article")." where article_id=67");
	$smarty->assign('jmxz', $jmxz);
	// 登陆注册-注册类型
