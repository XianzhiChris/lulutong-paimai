...
//		}
		...
		//类别幻灯片  李云鹏20160427
		$smarty->assign('sort_ad',      cat_flashimg_list($cat_id));
		///var_dump(cat_flashimg_list($cat_id));
...
/**
 * 获得分类下的商品  修改为分类下拍卖商品  李云鹏20160427
 *
 * @access  public
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $where .= " AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND ".
    ...
    $sql = "SELECT a.act_id, a.goods_id, a.act_name, a.ext_info,a.end_time,a.start_time,g.goods_id, g.goods_name, g.goods_name_style, g.click_count, g.goods_number, g.market_price,g.brand_id,g.supplier_id, " .
		   ...
		   " g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb2, g.goods_img " .
           " FROM " . $GLOBALS['ecs']->table('goods_activity') . " AS a," . $GLOBALS['ecs']->table('goods') .
		   " AS g " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $arr[$row['goods_id']]['short_act_name']       = $row['act_name'];
        }
        ...
        $arr[$row['goods_id']]['goods_thumb']      = get_image_path($row['goods_id'], $row['goods_thumb2'], true);
        $arr[$row['goods_id']]['goods_img']        = get_image_path($row['goods_id'], $row['goods_img']);
        $arr[$row['goods_id']]['url']              = build_uri('auction', array('auid'=>$row['act_id']), $row['goods_name']);
...
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='')
{
    ...
    /* 返回商品总数 */
    return $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' AS a,' . $GLOBALS['ecs']->table('goods') . ' AS g WHERE '. $where. $ext);
}
